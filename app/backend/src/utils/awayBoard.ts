...
import { IClubAway } from '../interfaces/IClubAway';

const leaderboardAway = async (): Promise<ILeaderBoard[]> => {
...
// verificou que todos os dados vem da tabela de matchs e clubs
  ...
  ) as unknown as IClubAway[];// retorno ok [array de time e suas partidas]
  ...
  const list = leaderboardTeams.map((team) => {
    ...
    team.awayTeam.forEach((t) => {
      ...
      goalsFavor += t.awayTeamGoals; // somou os gols marcados pelo time
      goalsOwn += t.homeTeamGoals; // somou os gols sofridos pelo time
    });
